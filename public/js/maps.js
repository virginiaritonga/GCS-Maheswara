...
$(document).ready(function () {
  ...
  $("#basemapslider").mousedown(function () {
    ...
    mapATS.dragging.disable();
  });

  $("#basemapslider").mouseup(function () {
    map.dragging.enable();
    mapATS.dragging.enable();
  });
  ...
  mapATS = L.map("mapATS").setView([-7.77116, 110.373283], 16.0);

  var mytile2 = L.tileLayer("/Map/Map/{z}/{x}/{y}.png", {
    maxZoom: 18,
    tms: false,
    attribution: "Generated by QTiles",
  }).addTo(mapATS);
  ...
  socket.on("arduino:data", function (dataSerial) {
	origin_latitude = parseFloat(dataSerial.origin_latitude);
    origin_longitude = parseFloat(dataSerial.origin_longitude);
    var circle = L.circle([origin_latitude, origin_longitude], {
      color: "red",
      fillColor: "#f03",
      fillOpacity: 0.5,
      radius: 10,
...
socket.on("arduino:data", function (dataSerial) {
  ...
  chart.data.datasets.forEach((dataset) => {
	lintang = parseFloat(dataSerial.lintangs);
    bujur = parseFloat(dataSerial.bujurs);
    if (lintang !== 0.0 && bujur !== 0.0) {
